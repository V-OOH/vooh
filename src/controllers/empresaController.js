...
var emailAPI = require("../services/email");
...
async function cadastrar(req, res) {
  ...
  if (cnpj == undefined) {
    ...
    return res
      .status(400)
      .json({ erro: "O campo Nome da Empresa está vazio!" });
  } else {
    empresaModel
      .cadastrar(nome, responsavel, cnpj)
...
function cadastrarContato(req, res) {
  ...
  if (celular == undefined) {
    return res
      .status(400)
      .json({ erro: "O campo Telefone Celular está vazio!" });
    ...
    empresaModel
      .cadastrarContato(celular, fixo, email, fkEmpresa)
      .then(function (resultado) {
        console.log("Cadastro realizado com sucesso no banco!");
